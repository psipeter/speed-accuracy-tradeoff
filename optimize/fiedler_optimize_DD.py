...
def run_DD(NDT, R, S, T, V, tmax, dt, rng, tiebreak='best'):
    ...
    elif tiebreak=='proportional':
        RT = max_samples
        probability = np.abs(dv[-1]/T)
        if dv[-1]>0:
            accuracy = 100 if rng.uniform(0, 1) < probability else 0
        elif dv[-1]<0:
            accuracy = 100 if rng.uniform(0, 1) > probability else 0            
    elif tiebreak=='best':
        RT = max_samples
...
def objective(trial, pid, dPs=[0.2], experiment_time=400, max_cues=12, dt=0.001, t_cue=0.1, rerun=False, params=None):
    ...
    if not rerun:
        ...
        mu_r0 = trial.suggest_float("mu_r0", 0.01, 0.03, step=0.001)  # R = mu_r0 * dP    
        sigma_r0 = trial.suggest_float("sigma_r0", 0.001, 0.01, step=0.001)
        # sigma_r0 = trial.suggest_categorical("sigma_r0", [0])
        mu_s = trial.suggest_categorical("mu_s", [0]) # mean of starting point distribution across trials is zero
        sigma_s = trial.suggest_float("sigma_s", 0.1, 3.0, step=0.1) # no variance in starting point
        # sigma_s = trial.suggest_categorical("sigma_s", [0]) # no variance in starting point
        V = trial.suggest_float("V", 0.05, 0.07, step=0.001) # drift variability
    else:
        T = params['T']
        mu_nd = params['mu_nd']
    ...
    for dP in dPs:
        ...
        while total_time < experiment_time:
            ...
            R = mu_r * dP
            accuracy, RT, dv = run_DD(NDT, R, S, T, V, 2*max_cues*t_cue, dt, rng)
